...
#pragma warning(                                                               \
    disable                                                                    \
    : 4996 ) // 'This function or variable may be unsafe': strcpy, strdup, sprintf, vsnprintf, sscanf, fopen
...
#pragma GCC diagnostic ignored                                                 \
    "-Wconversion" // warning: conversion to 'xxxx' from 'xxxx' may change value
#pragma GCC diagnostic ignored                                                 \
...
static void TextExRenderRightToLeft( ImVec2 pos, float align_width,
                                     ...
                                     const char* text_end, float wrap_width )
{
	...
	while( s < text_end )
	{
		...
		const char* line_end =
		    (const char*)ImMemchr( line_begin, '\n', text_end - line_begin );
		if( line_end == NULL )
			line_end = text_end;

		const char* segment_end = line_end;
		if( word_wrap_enabled )
			segment_end = g.Font->CalcWordWrapPosition( g.FontSize, line_begin,
			                                            line_end, wrap_width );

		const ImVec2 line_size =
		    ImGui::CalcTextSize( line_begin, segment_end, false, 0.0f );
		const float line_x = pos.x + ImMax( align_width - line_size.x, 0.0f );
		ImGui::RenderText( ImVec2( line_x, pos.y ), line_begin, segment_end,
		                   false );

		pos.y += line_height;
		if( segment_end < line_end )
		{
			s = ImTextCalcWordWrapNextLineStart( segment_end, line_end );
		}
		else
		{
                    ...
                    ImGuiTextFlags flags )
{
	...
	if( text_end - text <= 2000 || wrap_enabled || rtl_layout )
	{
		// Common case
		...
		if( rtl_layout )
			TextExRenderRightToLeft( bb.Min, rtl_align_width, text_begin,
			                         text_end, wrap_width );
		else
			...
			if( flags & ImGuiButtonFlags_PressedOnRelease )
			{
				if( mouse_button_released != -1 )
				{
					...
					if( !( flags & ImGuiButtonFlags_NoNavFocus ) )
						SetFocusID(
						    id,
						    window ); // FIXME: Lack of FocusWindow() call here is inconsistent with other paths. Research why.
					ClearActiveID();
				}
			}
...
void ImGui::ExtendHitBoxWhenNearViewportEdge( ImGuiWindow* window, ImRect* bb,
                                              float threshold, ImGuiAxis axis )
{
...
void ImGui::Scrollbar( ImGuiAxis axis )
{
	...
	ImDrawFlags rounding_corners = CalcRoundingFlagsForRectInRect(
	    bb, window->Rect(), g.Style.WindowBorderSize );
	float size_visible =
                         ...
                         ImDrawFlags draw_rounding_flags )
{
	...
	if( ( axis == ImGuiAxis_Y ) && bb_frame_height < bb_frame_width )
		alpha = ImSaturate( bb_frame_height /
		                    ImMax( bb_frame_width * 2.0f, 1.0f ) );
	if( alpha <= 0.0f )
		return false;
	...
	ImRect bb = bb_frame;
	float  padding =
	    IM_TRUNC( ImMin( style.ScrollbarPadding,
	                     ImMin( bb_frame_width, bb_frame_height ) * 0.5f ) );
	bb.Expand( -padding );
	...
	ExtendHitBoxWhenNearViewportEdge( window, &bb_hit, g.Style.WindowBorderSize,
	                                  (ImGuiAxis)( axis ^ 1 ) );
                   ...
                   const ImVec4& border_col )
{
	...
	ImageWithBg( tex_ref, image_size, uv0, uv1, ImVec4( 0, 0, 0, 0 ),
	             tint_col );
	PopStyleColor();
	PopStyleVar();
}
                         ...
                         const ImVec4& tint_col )
{
	...
	return ImageButtonEx( window->GetID( str_id ), tex_ref, image_size, uv0,
	                      uv1, bg_col, tint_col );
}
...
bool ImGui::Checkbox( const char* label, bool* v )
{
	...
	if( is_multi_select )
		MultiSelectItemFooter( id, &checked, &pressed,
		                       ImGuiMultiSelectFlags_CheckboxMode_ );
	else if( pressed )
		checked = !checked;
	...
	if( is_visible )
	{
		...
		RenderFrame( check_bb.Min, check_bb.Max, bg_col, true,
		             style.FrameRounding );
		if( mixed_value )
		{
                         ...
                         const char* overlay )
{
	...
	if( fill_x0 < fill_x1 )
		RenderRectFilledInRangeH( window->DrawList, bb,
		                          ...
		                          fill_x0, fill_x1, style.FrameRounding );
...
bool ImGui::TextLink( const char* label )
{
	...
	window->DrawList->AddLineH(
	    bb.Min.x, bb.Max.x, line_y, GetColorU32( line_colf ),
...
bool ImGui::TextLinkOpenURL( const char* label, const char* url )
{
	...
	SetItemTooltip(
	    ...
	    url ); // It is more reassuring for user to _always_ display URL when it is the same as the label.
                             ...
                             const char* label_end, float extra_w )
{
	...
	if( label_size.x > 0.0f )
	{
		...
		if( sep1_x2 > sep1_x1 && separator_thickness > 0.0f )
			window->DrawList->AddLineH( sep1_x1, sep1_x2, seps_y, separator_col,
			                            separator_thickness );
		if( sep2_x2 > sep2_x1 && separator_thickness > 0.0f )
			window->DrawList->AddLineH( sep2_x1, sep2_x2, seps_y, separator_col,
			                            separator_thickness );
		if( g.LogEnabled )
	...
	else
	{
		...
		if( separator_thickness > 0.0f )
			window->DrawList->AddLineH( sep1_x1, sep2_x2, seps_y, separator_col,
			                            separator_thickness );
	}
}
	...
	while( width_excess > 0.001f && count_same_width < count )
	{
		...
		for( int item_n = 0; item_n < count_same_width; item_n++ )
		{
			float width_to_remove_for_this_item =
			    ImMin( base_width_to_remove_per_item,
			           items[item_n].Width - width_min );
                   ...
                   const char* items_separated_by_zeros, int height_in_items )
{
	int         items_count = 0;
	const char* p =
	    items_separated_by_zeros; // FIXME-OPT: Avoid computing this, or at least only when combo is open
	while( *p )
	{
		p += ImStrlen( p ) + 1;
		items_count++;
                                   ...
                                   void* p_data_when_empty )
{
	...
	else
	{
		...
		if( format[0] == '\0' )
			format =
			    type_info
			        ->ScanFmt; // Format doesn't want us to show the number currently, but we still need to parse the resulting input
	}
	...
	if( is_logarithmic )
	{
		...
		float v_old_parametric =
		    ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
		        data_type, v_cur, v_min, v_max, logarithmic_zero_epsilon,
		        zero_deadzone_halfsize );
		float v_new_parametric = v_old_parametric + g.DragCurrentAccum;
		v_cur = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(
		    data_type, v_new_parametric, v_min, v_max, logarithmic_zero_epsilon,
		    zero_deadzone_halfsize );
...
static bool TempInputIsClampEnabled( ImGuiSliderFlags flags,
                                     ImGuiDataType data_type, const void* p_min,
                                     const void* p_max )
{
...
bool ImGui::DragScalar( const char* label, ImGuiDataType data_type,
...
float ImGui::ScaleRatioFromValueT( ImGuiDataType data_type, TYPE v, TYPE v_min,
                                   TYPE v_max, float logarithmic_zero_epsilon,
                             ...
                             ImGuiSliderFlags flags, ImRect* out_grab_bb )
{
	...
	const float v_range_f =
	    (float)( v_min < v_max
	                 ? v_max - v_min
	                 : v_min -
	                       v_max ); // We don't need high precision for what we do with it.
	...
	if( g.ActiveId == id )
	{
		         ...
		         g.ActiveIdSource == ImGuiInputSource_Gamepad )
		{
			...
			else if( g.SliderCurrentAccumDirty )
			{
				clicked_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(
				    data_type, *v, v_min, v_max, logarithmic_zero_epsilon,
				    zero_deadzone_halfsize );
		...
		if( set_new_value )
		{
			TYPE v_new = ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(
			    data_type, clicked_t, v_min, v_max, logarithmic_zero_epsilon,
			    zero_deadzone_halfsize );
                          ...
                          const char* format, ImGuiSliderFlags flags )
{
	...
	{
		// Only clamp Ctrl+Click input when ImGuiSliderFlags_ClampOnInput is set (generally via ImGuiSliderFlags_AlwaysClamp)
		const bool clamp_enabled = ( flags & ImGuiSliderFlags_ClampOnInput ) !=
...
bool ImGui::VSliderInt( const char* label, const ImVec2& size, int* v,
                                       ...
                                       size_t fmt_out_size )
{
	...
	while( fmt_in < fmt_end )
	{
		...
		if( c != '\'' && c != '$' &&
		    c !=
		        '_' ) // Custom flags provided by stb_sprintf.h. POSIX 2008 also supports '.
			*( fmt_out++ ) = c;
	}
                           ...
                           ImGuiInputTextCallback callback, void* user_data )
{
	...
	g.LastItemData.ItemFlags |=
	    ImGuiItemFlags_AllowDuplicateId; // Using ImGuiInputTextFlags_MergedItem above will skip ItemAdd() so we poke here.
	bool value_changed =
                             ...
                             const void* p_clamp_max )
{
	...
	char                     data_buf[32];
	format =
	    ImParseFormatTrimDecorations( format, fmt_buf, IM_COUNTOF( fmt_buf ) );
	if( format[0] == 0 )
	...
	if( !TempInputText( bb, id, label, data_buf, IM_COUNTOF( data_buf ),
	                    flags ) )
		return false;
                         ...
                         ImGuiInputTextFlags flags )
{
	...
	if( ( flags & ImGuiInputTextFlags_DisplayEmptyRefVal ) &&
		...
		DataTypeFormatString( buf, IM_COUNTOF( buf ), data_type, p_data,
		                      format );
	...
	bool value_changed =
	    input_edited ? DataTypeApplyFromText(
	...
	if( has_step_buttons )
	{
		...
		if( ButtonEx( "-", ImVec2( button_size, button_size ) ) )
		{
			DataTypeApplyOp( data_type, '-', p_data, p_data,
			                 g.IO.KeyCtrl && p_step_fast ? p_step_fast
			                                             : p_step );
...
bool ImGui::InputFloat( const char* label, float* v, float step,
...
static bool InputTextIsRightToLeft( ImGuiContext& g, ImGuiInputTextFlags flags )
{
	if( ( g.IO.ConfigFlags & ImGuiConfigFlags_RightToLeft ) == 0 )
		return false;
...
static float InputTextCalcTextWidth( ImGuiContext& g, const char* text_begin,
                                     const char* text_end )
{
	return InputTextCalcTextSize( &g, text_begin, text_end, text_end, NULL,
	                              NULL, ImDrawTextFlags_WrapKeepBlanks )
	    .x;
}

static float InputTextCalcRightToLeftOffset( ImGuiContext&     g,
                                             const ImGuiStyle& style,
                                             const ImVec2&     inner_size,
                                             const char*       text_begin,
                                             const char*       text_end )
{
	const float text_width = InputTextCalcTextWidth( g, text_begin, text_end );
	const float available_width =
	    ImMax( inner_size.x - style.FramePadding.x * 2.0f, 0.0f );
	return ImMax( available_width - text_width, 0.0f );
}

static void InputTextBuildGraphemeBoundaries( const char* text, int text_len,
                                              ImVector<int>* out_boundaries )
{
	out_boundaries->resize( 0 );
	out_boundaries->push_back( 0 );

	kbts_break_state break_state = {};
	kbts_BreakBegin( &break_state, KBTS_DIRECTION_DONT_KNOW,
	...
	for( const char* s = text; s < text_end; )
	{
		...
		kbts_BreakAddCodepoint( &break_state, (int)c, char_len,
		                        s + char_len >= text_end );
		kbts_break text_break;
		while( kbts_Break( &break_state, &text_break ) )
			if( ( text_break.Flags & KBTS_BREAK_FLAG_GRAPHEME ) &&
			    text_break.Position > 0 && text_break.Position < text_len )
				if( out_boundaries->back() != text_break.Position )
					out_boundaries->push_back( text_break.Position );
		s += char_len;
	}

	if( out_boundaries->back() != text_len )
		out_boundaries->push_back( text_len );
}

static int InputTextGetNextGraphemeBoundary( ImGuiInputTextState* obj, int idx )
{
	if( idx >= obj->TextLen )
		return obj->TextLen + 1;

	ImVector<int> boundaries;
	InputTextBuildGraphemeBoundaries( obj->TextSrc, obj->TextLen, &boundaries );
...
static int InputTextGetPrevGraphemeBoundary( ImGuiInputTextState* obj, int idx )
{
	if( idx <= 0 )
		return -1;
	...
	ImVector<int> boundaries;
	InputTextBuildGraphemeBoundaries( obj->TextSrc, obj->TextLen, &boundaries );
	for( int n = boundaries.Size - 1; n >= 0; n-- )
		if( boundaries[n] < idx )
			return boundaries[n];
	return -1;
}

struct InputTextRtlGlyphInfo
{
	int            SourceOffset;
...
struct InputTextRtlCaretInfo
{
	int   ByteOffset;
	float X;
};

static kbts_shape_context* InputTextGetRtlShapeContext( ImFont* font )
{
	...
	for( ImFontConfig* src : font->Sources )
		kbts_ShapePushFontFromMemory( ctx, src->FontData, src->FontDataSize,
		                              (int)src->FontNo );
	font->RtlShapeContext = ctx;
	return ctx;
}

static float InputTextGetRtlFontUnitScale( ImGuiContext& g )
{
	ImFontConfig* src = g.Font->Sources.Size > 0 ? g.Font->Sources[0] : NULL;
	if( src == NULL || src->FontLoaderData == NULL )
		return g.FontBakedScale;

	return ImFontRtlGetFontUnitScale( g.Font, g.FontBaked );
}

static int InputTextFindBoundaryIndex( const ImVector<int>& boundaries,
                                       int                  byte_offset )
{
	int boundary_n = 0;
	while( boundary_n + 1 < boundaries.Size &&
	       boundaries[boundary_n + 1] <= byte_offset )
		boundary_n++;
	return boundary_n;
}

// Shape 'text' and return glyphs in visual (left-to-right) order, matching the
// layout produced by ImFontRtlRenderText(). Fills 'out_visual_glyphs' and
// (optionally) the resolved paragraph direction. Returns the total advance width.
static float InputTextBuildRightToLeftVisualGlyphs(
    ImGuiContext& g, const char* text_begin, const char* text_end,
    ImVector<InputTextRtlGlyphInfo>* out_visual_glyphs,
    kbts_direction*                  out_paragraph_direction = NULL )
{
	out_visual_glyphs->resize( 0 );
	if( out_paragraph_direction != NULL )
		*out_paragraph_direction = KBTS_DIRECTION_LTR;

	const int text_len = (int)( text_end - text_begin );
	if( text_len <= 0 )
		return 0.0f;

	kbts_shape_context* ctx = InputTextGetRtlShapeContext( g.Font );
	kbts_ShapeBegin( ctx, KBTS_DIRECTION_DONT_KNOW, KBTS_LANGUAGE_DONT_KNOW );
	kbts_ShapeUtf8( ctx, text_begin, text_len,
	                KBTS_USER_ID_GENERATION_MODE_SOURCE_INDEX );
	kbts_ShapeEnd( ctx );

	const float scale_for_layout = InputTextGetRtlFontUnitScale( g );
	float       total_width      = 0.0f;
	ImVector<InputTextRtlGlyphInfo> logical_glyphs;
	ImVector<InputTextRtlRunInfo>   runs;

	kbts_run run;
	while( kbts_ShapeRun( ctx, &run ) )
	{
		InputTextRtlRunInfo run_info;
		run_info.GlyphBegin         = logical_glyphs.Size;
		run_info.ParagraphDirection = run.ParagraphDirection;
		...
		while( kbts_GlyphIteratorNext( &run.Glyphs, &glyph ) )
		{
			...
			kbts_shape_codepoint shape_codepoint;
			if( !kbts_ShapeGetShapeCodepoint(
			        ctx, glyph->UserIdOrCodepointIndex, &shape_codepoint ) )
				continue;

			InputTextRtlGlyphInfo glyph_info;
			glyph_info.SourceOffset = shape_codepoint.UserId;
			glyph_info.Direction    = run.Direction;
			glyph_info.AdvanceX     = (float)glyph->AdvanceX * scale_for_layout;
			logical_glyphs.push_back( glyph_info );
			total_width += glyph_info.AdvanceX;
		}

		run_info.GlyphEnd = logical_glyphs.Size;
		if( run_info.GlyphBegin != run_info.GlyphEnd )
			runs.push_back( run_info );
	}

	if( out_paragraph_direction != NULL && runs.Size > 0 )
		*out_paragraph_direction = runs[0].ParagraphDirection;

	if( runs.Size > 1 && runs[0].ParagraphDirection == KBTS_DIRECTION_RTL )
	{
		for( int run_n = runs.Size - 1; run_n >= 0; run_n-- )
			for( int glyph_n = runs[run_n].GlyphBegin;
			     glyph_n < runs[run_n].GlyphEnd; glyph_n++ )
				out_visual_glyphs->push_back( logical_glyphs[glyph_n] );
	}
	else
	{
		out_visual_glyphs->swap( logical_glyphs );
	}
...
static float
InputTextBuildRightToLeftCaretMap( ImGuiContext& g, const char* text_begin,
                                   const char*                      text_end,
                                   ImVector<InputTextRtlCaretInfo>* out_carets )
{
	out_carets->resize( 0 );
	const int text_len = (int)( text_end - text_begin );
	if( text_len <= 0 )
	{
		out_carets->push_back( { 0, 0.0f } );
		return 0.0f;
	}

	ImVector<int> boundaries;
	InputTextBuildGraphemeBoundaries( text_begin, text_len, &boundaries );
	...
	for( float& x : boundary_x )
		x = -FLT_MAX;

	float x = 0.0f;
	for( const InputTextRtlGlyphInfo& glyph : visual_glyphs )
	{
		const int boundary_n =
		    InputTextFindBoundaryIndex( boundaries, glyph.SourceOffset );
		const int next_boundary_n =
		    ImMin( boundary_n + 1, boundaries.Size - 1 );
		if( glyph.Direction == KBTS_DIRECTION_RTL )
		{
			boundary_x[boundary_n]      = x + glyph.AdvanceX;
			boundary_x[next_boundary_n] = x;
		}
		else
		{
			boundary_x[boundary_n]      = x;
			boundary_x[next_boundary_n] = x + glyph.AdvanceX;
		}
		x += glyph.AdvanceX;
	}

	if( boundary_x[0] == -FLT_MAX )
		boundary_x[0] =
		    ( paragraph_direction == KBTS_DIRECTION_RTL ) ? total_width : 0.0f;
	for( int n = 1; n < boundary_x.Size; n++ )
		if( boundary_x[n] == -FLT_MAX )
			boundary_x[n] = boundary_x[n - 1];

	out_carets->resize( boundaries.Size );
	for( int n = 0; n < boundaries.Size; n++ )
	{
		( *out_carets )[n].ByteOffset = boundaries[n];
		( *out_carets )[n].X          = boundary_x[n];
	}

	return total_width;
}

static float InputTextGetRightToLeftCaretX( ImGuiContext& g,
                                            const char*   text_begin,
                                            const char*   text_end,
                                            int           byte_offset )
{
	ImVector<InputTextRtlCaretInfo> carets;
	InputTextBuildRightToLeftCaretMap( g, text_begin, text_end, &carets );
	int best_n = 0;
	for( int n = 0; n < carets.Size; n++ )
	{
		if( carets[n].ByteOffset == byte_offset )
			return carets[n].X;
		if( carets[n].ByteOffset < byte_offset )
			best_n = n;
	}
	return carets[best_n].X;
}

// Compute the visual X-intervals covering the logical byte range
// [sel_begin, sel_end) for right-to-left / bidi text. A logically contiguous
// selection can map to several disjoint visual rectangles (one per visual run),
// so we emit one { min_x, max_x } interval per contiguous run of selected glyphs.
static void InputTextGetRightToLeftSelectionRanges(
    ImGuiContext& g, const char* text_begin, const char* text_end,
    int sel_begin, int sel_end, ImVector<ImVec2>* out_ranges )
{
	out_ranges->resize( 0 );
	...
	{
		const float x0 = x;
		const float x1 = x + glyph.AdvanceX;
		const bool  selected =
		    ( glyph.SourceOffset >= sel_begin && glyph.SourceOffset < sel_end );
		if( selected )
		{
			if( !has_run )
			{
			...
			{
				run_min = ImMin( run_min, x0 );
				run_max = ImMax( run_max, x1 );
			}
		}
		else if( has_run )
		{
			out_ranges->push_back( ImVec2( run_min, run_max ) );
			has_run = false;
		}
		x = x1;
	}
...
static int InputTextFindRightToLeftCaretFromX( ImGuiContext& g,
                                               const char*   text_begin,
                                               const char* text_end, float x )
{
	ImVector<InputTextRtlCaretInfo> carets;
	InputTextBuildRightToLeftCaretMap( g, text_begin, text_end, &carets );
	...
	for( const InputTextRtlCaretInfo& caret : carets )
	{
		const float dist = ImAbs( caret.X - x );
		if( dist < best_dist )
		{
			best_dist   = dist;
			best_offset = caret.ByteOffset;
		}
	}
	return best_offset;
}
...
{
	const char*  line_start = obj->TextSrc + line_start_idx;
	const char*  char_begin = line_start + char_idx;
	const char*  text_end   = obj->TextSrc + obj->TextLen;
	unsigned int c;
	const int    char_len = ImTextCharFromUtf8( &c, char_begin, text_end );
	if( (ImWchar)c == '\n' )
		return IMSTB_TEXTEDIT_GETWIDTH_NEWLINE;
	ImGuiContext& g = *obj->Ctx;
	if( ImFontRtlTextNeedsShape( line_start, text_end ) )
	{
		const char* char_end = char_begin + char_len;
		const float x0 =
		    InputTextCalcTextSize( &g, line_start, char_begin, text_end, NULL,
		                           NULL, ImDrawTextFlags_WrapKeepBlanks )
		        .x;
		const float x1 =
		    InputTextCalcTextSize( &g, line_start, char_end, text_end, NULL,
		                           NULL, ImDrawTextFlags_WrapKeepBlanks )
		        .x;
		return ImMax( x1 - x0, 0.0f );
	}
...
static int IMSTB_TEXTEDIT_GETNEXTCHARINDEX_IMPL( ImGuiInputTextState* obj,
                                                 int                  idx )
{
	return InputTextGetNextGraphemeBoundary( obj, idx );
}

static int IMSTB_TEXTEDIT_GETPREVCHARINDEX_IMPL( ImGuiInputTextState* obj,
                                                 int                  idx )
{
	return InputTextGetPrevGraphemeBoundary( obj, idx );
}
...
static bool
InputTextFilterCharacter( ImGuiContext* ctx, ImGuiInputTextState* state,
                          unsigned int* p_char, ImGuiInputTextCallback callback,
...
static ImVec2
InputTextLineIndexGetPosOffset( ImGuiContext& g, ImGuiInputTextState* state,
                                ImGuiTextIndex* line_index, const char* buf,
...
static ImVec2 InputTextLineIndexGetRightToLeftPosOffset(
    ImGuiContext& g, const ImGuiStyle& style, const ImVec2& inner_size,
    ImGuiTextIndex* line_index, const char* buf, int cursor_n )
{
	const char* cursor_ptr = buf + cursor_n;
	int*        it_begin   = line_index->Offsets.begin();
	int*        it_end     = line_index->Offsets.end();
	...
	const float line_width = InputTextCalcTextWidth( g, line_start, line_end );
	const float align_offset = InputTextCalcRightToLeftOffset(
	    g, style, inner_size, line_start, line_end );

	ImVec2 offset;
	offset.x = align_offset +
	           InputTextGetRightToLeftCaretX(
                         ...
                         void*                  callback_user_data )
{
	...
	IM_ASSERT( !(
	    ...
	    ( flags &
	      ImGuiInputTextFlags_Multiline ) ) ); // Multiline does not not work with left-trimming
	...
	if( is_multiline )
	{
		...
		bool no_clip =
		    ...
		    ( id ==
		      g.NavActivateId ); // Mimic some of ItemAdd() logic + add InputTextDeactivatedState.ID check.
		...
		PushStyleVar(
		    ...
		    ImVec2(
		        0,
		        0 ) ); // Ensure no clip rect so mouse hover can reach FramePadding edges
		bool child_visible =
	...
	const bool init_changed_specs_readonly =
	    ( state != NULL &&
	      ( ( state->Flags ^ flags ) &
	        ImGuiInputTextFlags_ReadOnly ) ); // state != NULL means its our state.
	         ...
	         init_changed_specs_multiline || init_changed_specs_readonly )
	{
		...
		bool recycle_state =
		    ( state->ID == id && !init_changed_specs_multiline );
	...
	if( g.ActiveId == id )
	{
		...
		float      mouse_x =
		    ...
		    state->Scroll.x;
		float mouse_x_from_rtl_text = mouse_x;
		if( input_text_rtl )
		{
			const float align_offset = InputTextCalcRightToLeftOffset(
			    g, style, inner_size, state->TextSrc,
			    state->TextSrc + state->TextLen );
			const float text_width = InputTextCalcTextWidth(
			    g, state->TextSrc, state->TextSrc + state->TextLen );
			mouse_x_from_rtl_text = ( io.MousePos.x - frame_bb.Min.x -
			                          style.FramePadding.x - align_offset ) +
			                        state->Scroll.x;
			mouse_x               = text_width - mouse_x_from_rtl_text;
		}
		const float mouse_y =
		...
		else if( io.MouseClicked[0] && !state->SelectedAllMouseLock )
		{
			if( hovered )
			{
				if( input_text_rtl )
				{
					const int cursor_n = InputTextFindRightToLeftCaretFromX(
					    g, state->TextSrc, state->TextSrc + state->TextLen,
					    mouse_x_from_rtl_text );
					if( io.KeyShift )
					{
						if( !state->HasSelection() )
							state->Stb->select_start = state->Stb->cursor;
						state->Stb->cursor = state->Stb->select_end = cursor_n;
					}
					else
					{
						state->Stb->cursor         = state->Stb->select_start =
						    state->Stb->select_end = cursor_n;
					}
					ImStb::stb_textedit_clamp( state, state->Stb );
				}
				else if( io.KeyShift )
	...
	if( g.ActiveId == id )
	{
		                ...
		                ImGuiInputTextFlags_CallbackAlways ) ) != 0 )
		{
			...
			if( event_flag )
			{
				...
				state->Stb->select_end = ImClamp( callback_data.SelectionEnd, 0,
				                                  callback_data.BufTextLen );
				if( callback_data.BufDirty )
				{
	...
	if( is_multiline )
	{
		...
		bool will_scroll_y =
		    state && ( ( state->CursorFollow && render_cursor ) ||
		               ( state->CursorCenterY &&
	...
	const ImVec2 cursor_offset =
	    render_cursor && state
	        ? ( input_text_rtl ? InputTextLineIndexGetRightToLeftPosOffset(
	                                 g, style, inner_size, line_index,
	                                 buf_display, state->Stb->cursor )
	...
	const ImU32 text_col = GetColorU32(
	...
	if( render_cursor || render_selection )
	{
		...
		if( render_selection )
		{
			...
			const float bg_offy_up =
			    is_multiline
			        ? 0.0f
			        : -1.0f; // FIXME-DPI: those offsets should be part of the style? they don't play so well with multi-line selection.
			...
			ImVector<ImVec2> rtl_select_ranges;
			     ...
			     line_n++ )
			{
				...
				if( line_selected_begin < line_selected_end )
					rect_width +=
					    CalcTextSize( line_selected_begin, line_selected_end )
					        .x;
				...
				const float rect_y0 = draw_pos.y - draw_scroll.y +
				                      line_n * g.FontSize + bg_offy_up;
				const float rect_y1 = draw_pos.y - draw_scroll.y +
				                      line_n * g.FontSize + bg_offy_dn +
				                      g.FontSize;
				if( input_text_rtl )
				{
					// Bidi text: a logically contiguous selection can map to
					// several disjoint visual runs, so draw one rect per run.
					const float align_offset = InputTextCalcRightToLeftOffset(
					    g, style, inner_size, p, p_eol );
					const float base_x =
					    draw_pos.x - draw_scroll.x + align_offset;
					InputTextGetRightToLeftSelectionRanges(
					    g, p, p_eol, (int)( line_selected_begin - p ),
					    (int)( line_selected_end - p ), &rtl_select_ranges );
					for( const ImVec2& range : rtl_select_ranges )
					{
						ImRect rect( base_x + range.x, rect_y0,
						             base_x + range.y, rect_y1 );
	...
	if( g.ActiveId != id && ( flags & ImGuiInputTextFlags_ElideLeft ) &&
	    !render_cursor && !render_selection )
		draw_pos.x = ImMin(
		    draw_pos.x, frame_bb.Max.x - CalcTextSize( buf_display, NULL ).x -
		                    style.FramePadding.x );
	//draw_scroll.x = state->Scroll.x; // Preserve scroll when inactive?

	// Render text
	const float input_text_rtl_render_offset =
	    input_text_rtl
	        ? InputTextCalcRightToLeftOffset(
	              g, style, inner_size,
	...
	if( render_cursor )
	{
		...
		if( cursor_is_visible && cursor_screen_rect.Overlaps( clip_rect ) )
			draw_window->DrawList->AddLineV(
			    cursor_screen_rect.Min.x, cursor_screen_rect.Min.y,
...
void ImGui::DebugNodeInputTextState( ImGuiInputTextState* state )
{
	...
	Text(
	    "TextLen: %d, Cursor: %d%s, Selection: %d..%d", state->TextLen,
	    stb_state->cursor,
	                    ...
	                    ImGuiChildFlags_ResizeY ) ) // Visualize undo state
	{
		...
		for( int n = 0; n < IMSTB_TEXTEDIT_UNDOSTATECOUNT; n++ )
		{
			...
			Text( "%c [%02d] where %03d, insert %03d, delete %03d, "
			      "char_storage %03d \"%.*s\"",
                        ...
                        ImGuiColorEditFlags flags )
{
	         ...
	         ( flags & ImGuiColorEditFlags_NoInputs ) == 0 )
	{
		               ...
		               ImGuiInputTextFlags_CharsUppercase ) )
		{
			...
			i[0] = i[1] = i[2] = 0;
			i[3] =
			    0xFF; // alpha default to 255 is not parsed by scanf (e.g. inputting #FFFFFF omitting alpha)
			int r;
			if( alpha )
	...
	if( ( flags & ImGuiColorEditFlags_NoBorder ) == 0 )
	{
		if( g.Style.FrameBorderSize > 0.0f )
			...
			window->DrawList->AddRect(
			    ...
			    1.0f *
			        (float)(int)g.Style
			            ._MainScale ); // Color buttons are often in need of some sort of border // FIXME-DPI
	}
...
void ImGui::ColorEditOptionsPopup( const float* col, ImGuiColorEditFlags flags )
{
	...
	if( BeginPopup( "Copy" ) )
	{
		...
		ImFormatString( buf, IM_COUNTOF( buf ), "(%d,%d,%d,%d)", cr, cg, cb,
		                ca );
		if( Selectable( buf ) )
			SetClipboardText( buf );
	...
	if( !is_visible )
	{
		      ...
		      ( 1 << ( window->DC.TreeDepth - 1 ) ) ) )
		{
			...
			parent_data->DrawLinesToNodesY2 = ImMax(
			    ...
			    window->DC.CursorPos
			        .y ); // Don't need to aim to mid Y position as we are clipped anyway.
...
void ImGui::TreeNodeDrawLineToChildNode( const ImVec2& target_pos )
{
	...
	{
		x1 += 0.5f + rounding;
		window->DrawList->PathArcToFast( ImVec2( x1, y - rounding ), rounding,
		                                 6, 3 );
		if( x1 < x2 )
	...
	{
		window->DrawList->AddLineH( x1, x2, y,
...
void ImGui::TreeNodeDrawLineToTreePop( const ImGuiTreeNodeStackData* data )
{
	...
	if( data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull )
	{
		...
		y2_full =
		    ImTrunc( y2_full - g.Style.ItemSpacing.y - g.FontSize * 0.5f );
	...
	window->DrawList->AddLineV( x, y1, y2, GetColorU32( ImGuiCol_TreeLines ),
	                            g.Style.TreeLinesSize );
	if( data->DrawLinesTableColumn != -1 )
                        ...
                        ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
	...
	{
		GetForegroundDrawList()->AddText( pos, IM_COL32( 255, 255, 0, 200 ),
		                                  label, label_end );
	}
#endif

	// Automatically close popups
	if( pressed && !auto_selected &&
    ...
    int nav_item_idx )
{
	...
	for( int idx = 0; idx < items_count; idx++ )
	{
		...
		if( first_match_idx == -1 ) // Record first match for wrapping.
			first_match_idx = idx;
		if( nav_item_idx ==
		    idx ) // Record that we encountering nav_item so we can return next match.
	                 ...
	                 ImGuiMultiSelectFlags_BoxSelect2d ) )
	{
		...
		if( bs->UnclipMode && g.CurrentTable != NULL )
			TableApplyExternalUnclipRect(
			    g.CurrentTable, bs->UnclipRect ); // No need submitting both
	}
		...
		for( ImRect& unclip_r : bs->UnclipRects )
			if( !unclip_r.IsInverted() )
				GetForegroundDrawList()->AddRect(
				    unclip_r.Min, unclip_r.Max,
				    bs->UnclipMode ? IM_COL32( 255, 255, 0, 200 )
				                   : IM_COL32( 255, 0, 0, 200 ),
				    0.0f, 0, 4.0f );
		GetForegroundDrawList()->AddRect(
		    bs->UnclipRect.Min, bs->UnclipRect.Max,
		    bs->UnclipMode ? IM_COL32( 255, 255, 0, 200 )
		                   : IM_COL32( 255, 0, 0, 200 ),
		    0.0f, 0, 2.0f );
	}
                          ...
                          ImGuiMultiSelectFlags ms_flags )
{
	...
	bs->EndPosRel = WindowPosAbsToRel(
	    window,
	    ImClamp(
	        g.IO.MousePos, scope_rect.Min,
	        scope_rect
	            .Max ) ); // Clamp stored position according to current scrolling view
                                             ...
                                             int items_count )
{
	...
	ms->IsSoleOrUnknownSelectionSize = ( storage->LastSelectionSize == 1 ) ||
	                                   ( storage->LastSelectionSize == -1 );
                                   ...
                                   ImGuiMultiSelectFlags extra_flags )
{
	...
	if( ms->BoxSelectId != 0 )
		if( ImGuiBoxSelectState* bs = GetBoxSelectState( ms->BoxSelectId ) )
		{
			...
			if( !window->DC
			         .NavIsScrollPushableX ) // FIXME: Rename to be more generic.
				if( ImGuiTable* table = g.CurrentTable )
					if( table->CurrentColumn != -1 )
					{
						...
						if( clip_min_x !=
						    clip_max_x ) // When zero sized we expect that bounds have been clamped and thus are unreliable
						{
							item_rect.Min.x =
    ...
    int size_before_amends, int selection_order )
{
	...
	if( selected && !is_contained )
		storage->Data.push_back( ImGuiStoragePair(
		    id,
		    selection_order ) ); // Push unsorted at end of vector, will be sorted in SelectionMultiAmendsFinish()
...
bool ImGui::BeginMenuEx( const char* label, const char* icon, bool enabled )
{
	...
	else
	{
		...
		float min_w       = offsets->DeclColumns(
	...
	if( menu_is_open )
	{
		...
		PushStyleVar(
		    ...
		    style
		        .PopupRounding ); // First level will use _PopupRounding, subsequent will use _ChildRounding
		menu_is_open = BeginPopupMenuEx(
		    id, label,
                        ...
                        const char* shortcut, bool selected, bool enabled )
{
	...
	if( window->DC.LayoutType == ImGuiLayoutType_Horizontal )
	{
		...
		ImVec2 text_pos( window->DC.CursorPos.x + offsets->OffsetLabel,
		                 window->DC.CursorPos.y +
		                     window->DC.CurrLineTextBaseOffset );
		PushStyleVarX( ImGuiStyleVar_ItemSpacing, style.ItemSpacing.x * 2.0f );
		pressed = Selectable( "", selected, selectable_flags,
		                      ImVec2( label_size.x, 0.0f ) );
		PopStyleVar();
	...
	else
	{
		...
		float checkmark_w = IM_TRUNC(
		    g.FontSize *
		    1.20f ); // FIXME: Always accounted for, even in a menu with none, because 'selected' has no neutral setting.
		float min_w =
		    offsets->DeclColumns( icon_w, label_size.x, shortcut_w,
		...
		if( g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible )
		{
			RenderText( text_pos + ImVec2( offsets->OffsetLabel, 0.0f ), label,
			            label_end, false );
			if( icon_w > 0.0f )
				RenderText( text_pos + ImVec2( offsets->OffsetIcon, 0.0f ),
				            icon );
			if( shortcut_w > 0.0f )
			{
				PushStyleColor( ImGuiCol_Text,
                       ...
                       ImGuiTabItemFlags flags, ImGuiWindow* docked_window )
{
	...
	if( want_clip_rect )
		PushClipRect( ImVec2( ImClamp( bb.Min.x, tab_bar->ScrollingRectMinX,
		                               tab_bar->ScrollingRectMaxX ),
		                      bb.Min.y - 1 ),
                               ...
                               ImGuiTabItemFlags flags, ImU32 col )
{
	...
	draw_list->AddRectFilled( bb.Min, ImVec2( bb.Max.x, y2 ), col, rounding,
                                        ...
                                        bool* out_text_clipped )
{
	...
	RenderTextEllipsis( draw_list, text_ellipsis_clip_bb.Min,
	                    text_ellipsis_clip_bb.Max, ellipsis_max_x, label,
