...
#define IM_ASSERT( _EXPR )                                                     \
	assert(                                                                    \
	    _EXPR ) // You can override the default assert handler by editing imconfig.h
#endif
#define IM_COUNTOF( _ARR )                                                     \
	( (int)( sizeof( _ARR ) /                                                  \
	         sizeof( *(                                                        \
	             _ARR ) ) ) ) // Size of a static C-style array. Don't use on pointers!
...
struct
    ImFontAtlasRect; // Output of ImFontAtlas::GetCustomRect() when using custom rectangles.
...
struct
    ImFontLoader; // Opaque interface to a font loading backend (stb_truetype, FreeType etc.).
struct
    ImTextureData; // Specs and pixel storage for a texture used by Dear ImGui.
...
struct
    ImGuiIO; // Main configuration and I/O between your application and ImGui (also see: ImGuiPlatformIO)
struct
...
struct
    ImGuiOnceUponAFrame; // Helper for running a block of code not more than once a frame
...
typedef int
    ImGuiConfigFlags; // -> enum ImGuiConfigFlags_     // Flags: for io.ConfigFlags
typedef int
...
typedef int
    ImGuiListClipperFlags; // -> enum ImGuiListClipperFlags_// Flags: for ImGuiListClipper
typedef int
...
typedef int
    ImGuiTabBarFlags; // -> enum ImGuiTabBarFlags_     // Flags: for BeginTabBar()
typedef int
...
IMGUI_API void ShowAboutWindow(
    bool* p_open =
        NULL ); // create About window. display Dear ImGui version, credits and build/system information.
...
IMGUI_API void PushFont(
    ImFont* font,
    float
        font_size_base_unscaled ); // Use NULL as a shortcut to keep current font. Use 0.0f to keep current size.
...
IMGUI_API void PushStyleColor(
    ImGuiCol idx,
    ImU32
        col ); // modify a style color. always use this if you modify the style after NewFrame().
IMGUI_API void PushStyleColor( ImGuiCol idx, const ImVec4& col );
IMGUI_API void PopStyleColor( int count = 1 );
IMGUI_API void PushStyleVar(
    ImGuiStyleVar idx,
    float
        val ); // modify a style float variable. always use this if you modify the style after NewFrame()!
IMGUI_API void
...
IMGUI_API void PushItemFlag(
    ...
    bool
        enabled ); // modify specified shared item flag, e.g. PushItemFlag(ImGuiItemFlags_NoTabStop, true)
...
IMGUI_API void SetCursorScreenPos(
    const ImVec2&
        pos ); // cursor position, absolute coordinates. THIS IS YOUR BEST FRIEND.
IMGUI_API ImVec2
...
IMGUI_API void TextUnformatted(
    const char* text,
    const char* text_end =
        NULL ); // raw text without formatting. Practically equivalent to 'Text("%s", text)' but doesn't require null terminated string if 'text_end' is specified.
IMGUI_API void Text( const char* fmt, ... ) IM_FMTARGS( 1 ); // formatted text
IMGUI_API void TextV( const char* fmt, va_list args ) IM_FMTLIST( 1 );
IMGUI_API void
TextColored( const ImVec4& col, const char* fmt, ... ) IM_FMTARGS(
...
IMGUI_API bool SmallButton(
    const char*
        label ); // button with (FramePadding.y == 0) to easily embed within text
...
IMGUI_API bool RadioButton(
    const char* label,
    bool
        active ); // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
IMGUI_API bool RadioButton(
...
IMGUI_API void SetNextItemStorageID(
    ImGuiID
        storage_id ); // set id to use for open/close storage (default to same as item id).
IMGUI_API bool
TreeNodeGetOpen( ImGuiID storage_id ); // retrieve tree node open/close state.
...
IMGUI_API void OpenPopupOnItemClick(
    ...
    ImGuiPopupFlags popup_flags =
        0 ); // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
IMGUI_API void
...
IMGUI_API bool BeginPopupContextItem(
    const char*     str_id = NULL,
    ImGuiPopupFlags popup_flags =
        0 ); // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
...
IMGUI_API bool BeginPopupContextVoid(
    const char*     str_id = NULL,
    ImGuiPopupFlags popup_flags =
        0 ); // open+begin popup when clicked in void (where there are no windows).
...
IMGUI_API void TableNextRow(
    ...
    float              min_row_height =
        0.0f ); // append into the first cell of a new row. 'min_row_height' include the minimum top and bottom padding aka CellPadding.y * 2.0f.
IMGUI_API bool
...
// Overlapping mode
IMGUI_API void
SetNextItemAllowOverlap(); // allow next item to be overlapped by a subsequent item. Typically useful with InvisibleButton(), Selectable(), TreeNode() covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.
...
IMGUI_API int            GetItemClickedCountWithSingleClickDelay(
    ...
    float            delay =
        -1.0f ); // [BETA] building block for disambiguation between single-click and double-click. Returns 1 on single-click but delayed by io.MouseSingleClickDelay after mouse release. Returns 2+ on double-click or repeated clicks.
...
IMGUI_API bool IsKeyPressed(
    ImGuiKey key,
    bool     repeat =
        true ); // was key pressed (went from !Down to Down)? Repeat rate uses io.KeyRepeatDelay / KeyRepeatRate.
IMGUI_API bool
...
IMGUI_API bool SetItemKeyOwner(
    ImGuiKey
        key ); // Set key owner to last item ID if it is hovered or active. Return true when ownership has been set. Roughly equivalent to 'if (TestKeyOwner(key, GetItemID()) && (IsItemHovered() || IsItemActive())) { SetKeyOwner(key, GetItemID());'.
...
IMGUI_API bool IsMouseReleasedWithDelay(
    ImGuiMouseButton button,
    float            delay =
        -1.f ); // delayed mouse release. Use sparingly. Prefer higher-level helper GetItemClickedCountWithSingleClickDelay(). Generally used with 'delay >= io.MouseDoubleClickTime' + combined with a 'io.MouseClickedLastCount==1' test.
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_NoNavFocus =
	    1
	    << 17, // No focusing toward this window with keyboard/gamepad navigation (e.g. skipped by Ctrl+Tab)
...
enum ImGuiItemFlags_
{
	...
	ImGuiItemFlags_Disabled =
	    1
	    << 6, // false    // [Internal] Disable interactions. DOES NOT affect visuals. This is used by BeginDisabled()/EndDisabled() and only provided here so you can read back via GetItemFlags().
};
...
enum ImGuiInputTextFlags_
{
	...
	ImGuiInputTextFlags_CtrlEnterForNewLine =
	    1
	    << 8, // In multi-line mode: validate with Enter, add new line with Ctrl+Enter (default is opposite: validate with Ctrl+Enter, add line with Enter). Note that Shift+Enter always enter a new line either way.
	...
	ImGuiInputTextFlags_ElideLeft =
	    1
	    << 17, // When text doesn't fit, elide left side to ensure right side stays visible. Useful for path/filenames. Single-line only!
	...
	ImGuiInputTextFlags_CallbackAlways =
	    1
	    << 20, // Callback on each iteration. User code may query cursor position, modify text buffer.
...
enum ImGuiTreeNodeFlags_
{
	...
	ImGuiTreeNodeFlags_AllowOverlap =
	    1
	    << 2, // Hit testing will allow subsequent widgets to overlap this one. Require previous frame HoveredId to match before being usable. Shortcut to calling SetNextItemAllowOverlap().
	...
	ImGuiTreeNodeFlags_Leaf =
	    1
	    << 8, // No collapsing, no arrow (use as a convenience for leaf nodes). Note: will always open a tree/id scope and return true. If you never use that scope, add ImGuiTreeNodeFlags_NoTreePushOnOpen.
	...
	ImGuiTreeNodeFlags_SpanAllColumns =
	    1
	    << 14, // Frame will span all columns of its container table (label will still fit in current column)
	...
	ImGuiTreeNodeFlags_DrawLinesToNodes =
	    1
	    << 20, // Horizontal lines to child nodes. Vertical line drawn down to bottom-most child node. Slower (for large trees).
...
enum ImGuiPopupFlags_
{
	...
	ImGuiPopupFlags_MouseButtonLeft =
	    1
	    << 2, // For BeginPopupContext*(): open on Left Mouse release. Only one button allowed!
	ImGuiPopupFlags_MouseButtonRight =
	    2
	    << 2, // For BeginPopupContext*(): open on Right Mouse release. Only one button allowed! (default)
	ImGuiPopupFlags_MouseButtonMiddle =
	    3
	    << 2, // For BeginPopupContext*(): open on Middle Mouse release. Only one button allowed!
	...
	ImGuiPopupFlags_AnyPopupId =
	    1
	    << 10, // For IsPopupOpen(): ignore the ImGuiID parameter and test for any popup.
...
enum ImGuiSelectableFlags_
{
	...
	ImGuiSelectableFlags_AllowOverlap =
	    1
	    << 4, // Hit testing will allow subsequent widgets to overlap this one. Require previous frame HoveredId to match before being usable. Shortcut to calling SetNextItemAllowOverlap().
	ImGuiSelectableFlags_Highlight =
	    1 << 5, // Make the item be displayed as if it is hovered
	ImGuiSelectableFlags_SelectOnNav =
	    1
	    << 6, // Auto-select when moved into, unless Ctrl is held. Automatic when in a BeginMultiSelect() block.
...
enum ImGuiTabBarFlags_
{
	...
	ImGuiTabBarFlags_FittingPolicyMixed =
	    1
	    << 7, // Shrink down tabs when they don't fit, until width is style.TabMinWidthShrink, then enable scrolling. Setting TabMinWidthShrink to FLT_MAX makes this behave like ImGuiTabBarFlags_FittingPolicyScroll.
...
enum ImGuiTabItemFlags_
{
	...
	ImGuiTabItemFlags_UnsavedDocument =
	    1
	    << 0, // Display a dot next to the title + set ImGuiTabItemFlags_NoAssumedClosure.
	...
	ImGuiTabItemFlags_NoReorder =
	    1
	    << 5, // Disable reordering this tab or having another tab cross over this tab
...
enum ImGuiDragDropFlags_
{
	...
	ImGuiDragDropFlags_AcceptDrawAsHovered =
	    1
	    << 13, // Accepting item will render as if hovered. Useful for e.g. a Button() used as a drop target.
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	ImGuiKey_COUNT =
	    ImGuiKey_NamedKey_END, // Obsoleted in 1.91.5 because it was misleading (since named keys don't start at 0 anymore)
	ImGuiMod_Shortcut =
...
enum ImGuiConfigFlags_
{
	...
	ImGuiConfigFlags_NavEnableKeyboard =
	    1
	    << 0, // Master keyboard navigation enable flag. Enable full Tabbing + directional arrows + Space/Enter to activate. Note: some features such as basic Tabbing and CtrL+Tab are enabled by regardless of this flag (and may be disabled via other means, see #4828, #9218).
...
enum ImGuiBackendFlags_
{
	...
	ImGuiBackendFlags_HasGamepad =
	    1
	    << 0, // Backend Platform supports gamepad and currently has one connected.
...
enum ImGuiButtonFlags_
{
	...
	ImGuiButtonFlags_AllowOverlap =
	    1
	    << 12, // Hit testing will allow subsequent widgets to overlap this one. Require previous frame HoveredId to match before being usable. Shortcut to calling SetNextItemAllowOverlap().
};
...
enum ImGuiColorEditFlags_
{
	...
	ImGuiColorEditFlags_NoPicker =
	    1
	    << 2, //              // ColorEdit: disable picker when clicking on color square.
	...
	ImGuiColorEditFlags_NoDragDrop =
	    1
	    << 9, //              // ColorEdit: disable drag and drop target/source. ColorButton: disable drag and drop source.
	ImGuiColorEditFlags_NoBorder =
	    1
	    << 10, //              // ColorButton: disable border (which is enforced by default)
	ImGuiColorEditFlags_NoColorMarkers =
	    1
	    << 11, //              // ColorEdit: disable rendering R/G/B/A color marker. May also be disabled globally by setting style.ColorMarkerSize = 0.
	...
	ImGuiColorEditFlags_AlphaOpaque =
	    1
	    << 12, //              // ColorEdit, ColorPicker, ColorButton: disable alpha in the preview,. Contrary to _NoAlpha it may still be edited when calling ColorEdit4()/ColorPicker4(). For ColorButton() this does the same as _NoAlpha.
	ImGuiColorEditFlags_AlphaNoBg =
	    1
	    << 13, //              // ColorEdit, ColorPicker, ColorButton: disable rendering a checkerboard background behind transparent color.
	ImGuiColorEditFlags_AlphaPreviewHalf =
	    1
	    << 14, //              // ColorEdit, ColorPicker, ColorButton: display half opaque / half transparent preview.

	// User Options (right-click on widget to change some of them).
	ImGuiColorEditFlags_AlphaBar =
	    1
	    << 18, //              // ColorEdit, ColorPicker: show vertical alpha bar/gradient in picker.
...
enum ImGuiSliderFlags_
{
	...
	ImGuiSliderFlags_ClampOnInput =
	    1
	    << 9, // Clamp value to min/max bounds when input manually with Ctrl+Click. By default Ctrl+Click allows going out of bounds.
	...
	ImGuiSliderFlags_ColorMarkers =
	    1
	    << 12, // DragScalarN(), SliderScalarN(): Draw R/G/B/A color markers on each component.
...
enum ImGuiTableFlags_
{
	...
	ImGuiTableFlags_Reorderable =
	    1
	    << 1, // Enable reordering columns in header row. (Need calling TableSetupColumn() + TableHeadersRow() to display headers, or using ImGuiTableFlags_ContextMenuInBody to access context-menu without headers).
	...
	ImGuiTableFlags_NoSavedSettings =
	    1
	    << 4, // Disable persisting columns order, width, visibility and sort settings in the .ini file.
	ImGuiTableFlags_ContextMenuInBody =
	    1
	    << 5, // Right-click on columns body/contents will also display table context menu. By default it is available in TableHeadersRow().
	// Decorations
...
enum ImGuiTableColumnFlags_
{
	...
	ImGuiTableColumnFlags_NoHeaderWidth =
	    1
	    << 13, // Disable header text width contribution to automatic column width.
...
{
	ImGuiID
	    ColumnUserID; // User data for the column (if specified by a TableSetupColumn() call in the 'ImGuiID user_data' field). FIXME: Should be called 'UserData'..
...
struct ImGuiStyle
{
	...
	float
	    FontScaleMain; // Main global scale factor. May be set by application once, or exposed to end-user.
	float
	    FontScaleDpi; // Additional global scale factor from viewport/monitor contents scale. In docking branch: when io.ConfigDpiScaleFonts is enabled, this is automatically overwritten when changing monitor DPI.
	...
	float
	    ScrollbarPadding; // Padding of scrollbar grab within its frame (same for both axes).
	float
	...
	float
	    TabCloseButtonMinWidthSelected; // -1: always visible. 0.0f: visible when hovered. >0.0f: visible when hovered if minimum width. FLT_MAX: never shrink, will behave like ImGuiTabBarFlags_FittingPolicyScroll.
	float
	...
	ImVec2
	    TableAngledHeadersTextAlign; // Alignment of angled headers within the cell
	ImGuiTreeNodeFlags
	...
	float
	    TreeLinesSize; // Thickness of outlines when using ImGuiTreeNodeFlags_DrawLines.
	float
	    TreeLinesRounding; // Radius of lines connecting child nodes to the vertical line.
	float MenuItemRounding; // Radius of MenuItem, BeginMenu rounding.
	float
	    SelectableRounding; // Radius of Selectable rounding. MODIFYING THIS IS DISCOURAGED. CONTIGUOUS SELECTIONS WILL NOT LOOK RIGHT. (#7589)
	float
	    DragDropTargetRounding; // Radius of the drag and drop target frame. When <0.0f: use FrameRounding.
	float
	    DragDropTargetBorderSize; // Thickness of the drag and drop target border.
	float
	    DragDropTargetPadding; // Size to expand the drag and drop target from actual target item size.
	float
	    ColorMarkerSize; // Size of R/G/B/A color markers for ColorEdit4() and for Drags/Sliders when using ImGuiSliderFlags_ColorMarkers.
	ImGuiDir
	...
	// [Internal]
	float
	    _MainScale; // FIXME-WIP: Reference scale, as applied by ScaleAllSizes(). PLEASE DO NOT USE THIS FOR NOW.
	float
	    _NextFrameFontSizeBase; // FIXME: Temporary hack until we finish remaining work.

	// Functions
	IMGUI_API      ImGuiStyle();
	IMGUI_API void ScaleAllSizes(
	    float
	        scale_factor ); // Scale all spacing/padding/thickness values. Do not scale fonts. See comments in definition. Consider not calling this if your initial scale factor if <1.0.
...
struct ImGuiIO
{
	...
	ImVec2
	    DisplaySize; // <unset>          // Main display size, in pixels (== GetMainViewport()->Size). May change every frame.
	ImVec2
	...
	bool
	    ConfigNavCaptureKeyboard; // = true           // Sets io.WantCaptureKeyboard when io.NavActive is set.
	bool
	...
	bool
	    ConfigNavCursorVisibleAlways; // = false          // Navigation cursor is always visible.

	// Ini Settings
	bool
	    ConfigIniSettingsSaveLastUsedDate; // = true         // Enable loading/saving last used day (YYYYMMDD) in some .ini struct, making things easier to audit and allowing custom tools to cleanup old data.
	...
	bool
	    ConfigInputTextEnterKeepActive; // = false          // [BETA] Pressing Enter will reactivate item and select all text (single-line only).
	bool
	...
	float
	    MouseDoubleClickTime; // = 0.30f          // Time for consecutive clicks to account as a double-click, in seconds.
	float
	    MouseDoubleClickMaxDist; // = 6.0f           // Distance threshold to stay in to validate a double-click or multiple clicks, in pixels.
	float
	    MouseSingleClickDelay; // = 0.60f          // Time for a delayed click when using GetItemClickedCountWithSingleClickDelay() or IsMouseReleasedWithDelay(), in seconds. Must be > io.MouseDoubleClickTime.
	float
	...
	bool
	    ConfigErrorRecoveryEnableDebugLog; // = true       // Enable debug log output on recoverable errors.
	bool
	...
	bool
	    ConfigDebugIsDebuggerPresent; // = false          // Enable various tools calling IM_DEBUG_BREAK().
	...
	bool
	    ConfigDebugHighlightIdConflictsShowItemPicker; //=true // Show "Item Picker" button in aforementioned popup.
	...
	void*
	    BackendPlatformUserData; // = NULL           // User data for platform backend
	void*
	    BackendRendererUserData; // = NULL           // User data for renderer backend
	void*
	...
	IMGUI_API void
	AddMouseButtonEvent( int button, bool down ); // Queue a mouse button change
	...
	bool
	    NavVisible; // Keyboard/Gamepad navigation highlight is visible and allowed (will handle ImGuiKey_NavXXX events).
	float
	...
	bool
	    KeySuper; // Keyboard modifier down: Windows/Super (non-macOS), Ctrl (macOS)
	...
	double MouseClickedTime
	    [5]; // Time of last click (used to figure out double-click)
	bool MouseClicked
	...
	bool
	    MouseCtrlLeftAsRightClick; // (OSX) Set to true when the current click was a Ctrl+Click that spawned a simulated right click
	float MouseDownDuration
	    [5]; // Duration the mouse button has been down (0.0f == just clicked)
	float MouseDownDurationPrev
	    [5]; // Previous time the mouse button has been down
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	float
	    FontGlobalScale; // Moved io.FontGlobalScale to style.FontScaleMain in 1.92.0 (June 2025)
...
struct ImGuiInputTextCallbackData
{
	...
	bool
	    EventActivated; // Input field just got activated       // Read-only    // [Always]
	bool
	    BufDirty; // Set if you modify Buf/BufTextLen!    // Write        // [Completion,History,Always]
	char*
...
enum ImGuiMultiSelectFlags_
{
	...
	ImGuiMultiSelectFlags_BoxSelectNoScroll =
	    1
	    << 8, // Disable scrolling when box-selecting and moving mouse near edges of scope.
	...
	ImGuiMultiSelectFlags_SelectOnAuto =
	    1
	    << 13, // Apply selection on mouse down when clicking on unselected item, on mouse up when clicking on selected item. (Default)
	ImGuiMultiSelectFlags_SelectOnClickAlways =
	    1
	    << 14, // Apply selection on mouse down when clicking on any items. Prevents Drag and Drop from being used on multiple-selection, but allows e.g. BoxSelect to always reselect even when clicking inside an existing selection. (Excel style behavior)
	ImGuiMultiSelectFlags_SelectOnClickRelease =
	    1
	    << 15, // Apply selection on mouse release when clicking an unselected item. Allow dragging an unselected item without altering selection.
	...
	ImGuiMultiSelectFlags_NoSelectOnRightClick =
	    1
	    << 17, // Disable default right-click processing, which selects item on mouse down, and is designed for context-menus.
...
struct ImGuiSelectionRequest
{
	...
	bool
	    Selected; //  ms:w, app:r     /  ms:w, app:r   // Parameter for SetAll/SetRange requests (true = select, false = unselect)
	ImS8
...
struct ImGuiSelectionBasicStorage
{
	...
	IMGUI_API void SetItemSelected(
	    ImGuiID id,
	    bool
	        selected ); // Add/remove an item from selection (generally done by ApplyRequests() function)
...
enum ImDrawFlags_
{
	...
	ImDrawFlags_RoundCornersTopLeft =
	    1
	    << 4, // Round top-left corner only (when rounding > 0.0f, we default to all corners).
	ImDrawFlags_RoundCornersTopRight =
	    1
	    << 5, // Round top-right corner only (when rounding > 0.0f, we default to all corners).
	ImDrawFlags_RoundCornersBottomLeft =
	    1
	    << 6, // Round bottom-left corner only (when rounding > 0.0f, we default to all corners).
	ImDrawFlags_RoundCornersBottomRight =
	    1
	    << 7, // Round bottom-right corner only (when rounding > 0.0f, we default to all corners).
	ImDrawFlags_RoundCornersNone =
	    1
	    << 8, // Disable rounding even if `float rounding > 0.0f`. This is NOT zero, NOT an implicit flag!
	...
	ImDrawFlags_Closed =
	    1
	    << 9, // PathStroke(), AddPolyline(): specify that shape should be closed.
...
enum ImDrawListFlags_
{
	...
	ImDrawListFlags_TextNoPixelSnap =
	    1
	    << 4, // Disable automatically snapping AddText() calls to pixel boundaries.
};
...
struct ImDrawList
{
	...
	IMGUI_API void AddLineH( float min_x, float max_x, float y, ImU32 col,
	                         float thickness = 1.0f );
	IMGUI_API void AddLineV( float x, float min_y, float max_y, ImU32 col,
	                         float thickness = 1.0f );
	IMGUI_API void
	...
	IMGUI_API void AddPolyline( const ImVec2* points, int num_points, ImU32 col,
	                            float thickness, ImDrawFlags flags = 0 );
	...
	inline void PathStroke( ImU32 col, float thickness = 1.0f,
	                        ImDrawFlags flags = 0 )
	{
	...
	IMGUI_API void AddCallback( ImDrawCallback callback, void* userdata = NULL,
	...
	// Obsolete names
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	inline void AddRect( const ImVec2& p_min, const ImVec2& p_max, ImU32 col,
	...
	inline void AddPolyline( const ImVec2* points, int num_points, ImU32 col,
	...
	void        AddPolyline( const ImVec2* points, int num_points, ImU32 col,
	                         ImDrawFlags flags, float thickness ) = delete;
...
{
	unsigned short x,
	    y; // Upper-left coordinates of rectangle to update, within the parent Pixels[] array.
...
struct ImTextureData
{
	...
	ImTextureStatus
	    Status; // rw   rw  // ImTextureStatus_OK/_WantCreate/_WantUpdates/_WantDestroy. Always use SetStatus() to modify!
	void*
	    BackendUserData; // -    rw  // Convenience storage for backend. Some backends may have enough with TexID.
	ImTextureID
	...
	ImTextureFormat
	    Format; // w    r   // ImTextureFormat_RGBA32 (default) or ImTextureFormat_Alpha8
	int Width;         // w    r   // Texture width
	int Height;        // w    r   // Texture height
	int BytesPerPixel; // w    r   // 4 or 1
	unsigned char*
	    Pixels; // w    r   // Pointer to whole texture buffer holding 'Width*Height' pixels and 'Width*Height*BytesPerPixels' bytes.
	ImTextureRect
	    UsedRect; // w    r   // Bounding box encompassing all past and queued Updates[].
	ImTextureRect
	    UpdateRect; // w    r   // Bounding box encompassing all queued Updates[].
...
struct ImFontConfig
{
	// Data Source
	char Name
	    [40]; // <auto>   // Name (strictly to ease debugging, hence limited size buffer)
	void* FontData;     //          // TTF/OTF data
	int   FontDataSize; //          // TTF/OTF data size
	bool
	    FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken by the owner ImFontAtlas (will delete memory itself). SINCE 1.92, THE DATA NEEDS TO PERSIST FOR WHOLE DURATION OF ATLAS.
	...
	bool
	    PixelSnapH; // false    // Align every glyph AdvanceX to pixel boundaries. Prevents fractional font size from working correctly! Useful e.g. if you are merging a non-pixel aligned font with the default font. If enabled, OversampleH/V will default to 1.
	ImS8
	...
	float
	    SizePixels; //          // Output size in pixels for rasterizer (more or less maps to the resulting font height).
	const ImWchar*
	...
	// [Internal]
	ImFontFlags
	    Flags; // Font flags (don't use just yet, will be exposed in upcoming 1.92.X updates)
	ImFont*
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
	bool
	    PixelSnapV; // true    // [Obsoleted in 1.91.6] Align Scaled GlyphOffset.y to pixel boundaries.
#endif
...
struct ImFontAtlas
{
	...
	IMGUI_API ImFont* AddFontDefault(
	    const ImFontConfig* font_cfg =
	        NULL ); // Selects between AddFontDefaultVector() and AddFontDefaultBitmap().
	...
	IMGUI_API ImFont* AddFontDefaultBitmap(
	    const ImFontConfig* font_cfg =
	        NULL ); // Embedded classic pixel-clean font. Recommended at Size 13px with no scaling.
	...
	IMGUI_API void
	Clear(); // Clear everything (fonts + textures). Don't call mid-frame!
	IMGUI_API void
	ClearFonts(); // Clear input+output font data/glyphs. New fonts and textures will be recreated afterwards.
	IMGUI_API void
	...
	bool
	    RendererHasTextures; // Copy of (BackendFlags & ImGuiBackendFlags_RendererHasTextures) from supporting context.
	bool
	    TexIsBuilt; // Set when texture was built matching current font input. Mostly useful for legacy IsBuilt() call.
	bool
	...
	ImVec2
	    TexUvWhitePixel; // Texture coordinates to a white pixel. May change as new texture gets created.
	ImVector<ImFont*>
	...
	IMGUI_API ImFontAtlasRectId AddCustomRectFontGlyphForSize(
	    ...
	    const ImVec2& offset = ImVec2( 0,
	                                   0 ) ); // ADDED AND OBSOLETED in 1.92.0
#endif
...
struct ImFontBaked
{
	...
	float
	    Size; // 4     // in  // Height of characters/line, set during loading (doesn't change after loading)
	...
	unsigned int LoadNoFallback
	    : 1; // 0  //     // Disable loading fallback in lower-level calls.
	unsigned int LoadNoRenderOnLayout
	    : 1; // 0  //     // Enable a two-steps mode where CalcTextSize() calls will load AdvanceX *without* rendering/packing glyphs. Only advantageous if you know that the glyph is unlikely to actually be rendered, otherwise it is slower because we'd do one query on the first CalcTextSize and one query on the first Draw.
	...
	FindGlyphNoFallback( ImWchar c ); // Return NULL if glyph doesn't exist
...
struct ImFont
{
	...
	ImVector<ImFontConfig*>
	    Sources; // 16    // in  // List of sources. Pointers within OwnerAtlas->Sources[]
	ImWchar
	    EllipsisChar; // 2-4   // out // Character used for ellipsis rendering ('...'). If you ever want to temporarily swap this for an alternative/dummy char, make sure to clear EllipsisAutoBake.
	ImWchar
	    FallbackChar; // 2-4   // out // Character used if a glyph isn't found (U+FFFD, '?')
	ImU8 Used8kPagesMap
	    [( IM_UNICODE_CODEPOINT_MAX + 1 ) / 8192 /
	     8]; // 1 bytes if ImWchar=ImWchar16, 17 bytes if ImWchar==ImWchar32. Store 1-bit for each block of 8K codepoints that has one active glyph. This is mainly used to facilitate iterations across all used codepoints.
	bool
	    EllipsisAutoBake; // 1     //     // Mark when the "..." glyph (== EllipsisChar) needs to be generated by combining multiple '.'.
	void*
	    RtlShapeContext; // 4-8   //     // Internal kb_text_shape context for Arabic shaping.
	ImGuiStorage
	...
	IMGUI_API void AddRemapChar(
	    ...
	    ImWchar
	        to_codepoint ); // Makes 'from_codepoint' character points to 'to_codepoint' glyph.
...
struct ImGuiPlatformIO
{
	...
	const char* ( *Platform_GetClipboardTextFn )(
	    ImGuiContext*
	        ctx ); // Should return NULL on failure (e.g. clipboard data is not text).
	...
	ImDrawCallback
	    DrawCallback_ResetRenderState; // Request to reset the graphics/render state.
	ImDrawCallback
	    DrawCallback_SetSamplerLinear; // Request backend to set texture sampling to Linear.
	ImDrawCallback
	    DrawCallback_SetSamplerNearest; // Request backend to set texture sampling to Nearest/Point.
	...
	IMGUI_API void
	ClearPlatformHandlers(); // Clear all Platform_XXX fields. Typically called on Platform Backend shutdown.
	IMGUI_API void
	ClearRendererHandlers(); // Clear all Renderer_XXX fields. Typically called on Renderer Backend shutdown.
};
...
#define ImDrawCallback_ResetRenderState                                        \
	( ImDrawCallback )(                                                        \
	    -8 ) // OBSOLETED in 1.92.8: Use ImGui::GetPlatformIO().DrawCallback_ResetRenderState
