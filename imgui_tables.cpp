                          ...
                          float inner_width )
{
	...
	if( use_child_window )
	{
		...
		BeginChildEx( name, instance_id, outer_rect.GetSize(),
		              child_child_flags, child_window_flags );
...
void ImGui::TableApplyQueuedRequests( ImGuiTable* table )
{
	...
	if( table->InstanceCurrent == 0 )
	{
		...
		{
			TableSetColumnDisplayOrder( table, table->ReorderColumn,
...
void ImGui::TableSetColumnDisplayOrder( ImGuiTable* table, int column_n,
                                        int dst_order )
{
...
static int TableGetMaxDisplayOrderAllowed( ImGuiTable* table, int src_order,
                                           int dst_order )
{
...
void ImGui::TableQueueSetColumnDisplayOrder( ImGuiTable* table, int column_n,
                                             int dst_order )
{
	...
	if( table->IsLayoutLocked &&
	    dst_order ==
	        src_order ) // We allow calling the function before layout w/ reconcile so don't early out.
		return;
                                   ...
                                   ImGuiTableColumnFlags flags_in )
{
	...
	else
	{
		IM_ASSERT( ImIsPowerOfTwo(
		    flags &
		    ImGuiTableColumnFlags_WidthMask_ ) ); // Check that only 1 of each set is used.
	}
...
void ImGui::TableUpdateLayout( ImGuiTable* table )
{
	...
	if( table->IsInitializing || table->IsSettingsRequestLoad )
	{
		...
		TableFixDisplayOrder(
		    table ); // Call even for non _Reorderable table as we loaded .ini data.
...
void ImGui::TableApplyExternalUnclipRect( ImGuiTable* table, ImRect& rect )
{
	...
	for( int column_n = 0; column_n < table->ColumnsCount; column_n++ )
	{
		...
		if( !column->IsRequestOutput )
			if( rect.Overlaps( ImRect( column->MinX, table->WorkRect.Min.y,
			                           column->MaxX, FLT_MAX ) ) )
...
void ImGui::EndTable()
{
	...
	const float inner_content_max_y = ImCeil(
	    table
	        ->RowPosY2 ); // Rounding final position is important as we currently don't round row height ('Demo->Tables->Outer Size' demo uses non-integer heights)
	...
	if( inner_window != outer_window )
	{
		...
		inner_window->DC.NavLayersActiveMask |=
		    1
		    << table
		           ->NavLayer; // So empty table don't appear to navigate differently.
...
static void TableSetupColumnApply( ImGuiTable* table, int idx, ImGuiID id,
                                   ImS16                 name_offset,
...
void ImGui::TableSetupColumn( const char* label, ImGuiTableColumnFlags flags,
                              float init_width_or_weight, ImGuiID user_data )
{
	ImGuiContext& g     = *GImGui;
	ImGuiTable*   table = g.CurrentTable;
	IM_ASSERT_USER_ERROR_RET(
	...
	IM_ASSERT_USER_ERROR_RET( table->IsLayoutLocked == false,
	                          ...
	                          "row!" ); // Table layout is locked when submitting a row or when calling BeginMultiSelect() with box-select.
	...
	{
		TableSetupColumnApply( table, column_idx, column_id, name_offset, flags,
...
void ImGui::TableReconcileColumns( ImGuiTable* table )
{
	...
	for( ImGuiTableReconcileColumnData& reconcile_data : reconcile_requests )
	{
		table->Columns[reconcile_data.ColumnNewIdx] =
		    reconcile_data
		        .ColumnOldData; // When old column was not found clear anyway with default-constructed data (which will set IsJustCreated=true)
		TableSetupColumnApply(
		    table, reconcile_data.ColumnNewIdx, reconcile_data.ID,
		    reconcile_data.NameOffset, reconcile_data.Flags,
		    reconcile_data.InitWidthOrWeight, reconcile_data.UserData );
		IMGUI_DEBUG_LOG_TABLE(
		    "[table] - old %d -> new %d \"%s\"\n", reconcile_data.ColumnOldIdx,
		    ...
		    TableGetColumnName(
		        table,
		        reconcile_data
		            .ColumnNewIdx ) ); // Log at the end so NameOffset was copied.
	}
...
void ImGui::TableBeginRow( ImGuiTable* table )
{
	...
	window->DC.CursorPosPrevLine      = ImVec2(
	    ...
	    window->DC.CursorPos.y +
	        table
	            ->RowCellPaddingY ); // This allows users to call SameLine() to share LineSize between columns.
...
void ImGui::TableEndRow( ImGuiTable* table )
{
	...
	if( is_visible )
	{
		...
		if( top_border_col && bg_y1 >= table->BgClipRect.Min.y &&
			...
			window->DrawList->AddLineH( table->BorderX1, table->BorderX2, bg_y1,
		...
		if( draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y &&
			...
			window->DrawList->AddLineH( table->BorderX1, table->BorderX2, bg_y2,
...
void ImGui::TableBeginCell( ImGuiTable* table, int column_n )
{
	...
	if( column->Flags & ImGuiTableColumnFlags_IndentEnable )
		start_x +=
		    table
		        ->RowIndentOffsetX; // ~~ += window.DC.Indent.x - table->HostIndentX, except we locked it for the row.
...
void ImGui::TableSetColumnWidth( int column_n, float width )
{
	...
	const float max_width = ImMax(
	    min_width,
	    column_0
	        ->WidthMax ); // Don't use TableCalcMaxColumnWidth() here as it would rely on MinX from last instance (#7933)
			...
			if( !( column->Flags & ImGuiTableColumnFlags_NoClip ) )
			{
				...
				if( !has_freeze_v )
					...
					content_max_x = ImMax(
					    column->ContentMaxXFrozen,
					    column
					        ->ContentMaxXHeadersUsed ); // Row freeze: use width before freeze
				else
					content_max_x =
					    column
...
void ImGui::TableDrawBorders( ImGuiTable* table )
{
	...
	if( table->Flags & ImGuiTableFlags_BordersOuter )
	{
		    ...
		    ImGuiTableFlags_BordersOuter )
		{
			inner_drawlist->AddRect( outer_border.Min, outer_border.Max,
			                         outer_col, 0.0f, border_size );
		}
		else if( table->Flags & ImGuiTableFlags_BordersOuterV )
		{
			inner_drawlist->AddLineV( outer_border.Min.x, outer_border.Min.y,
			                          outer_border.Max.y, outer_col,
			                          border_size );
			inner_drawlist->AddLineV( outer_border.Max.x, outer_border.Min.y,
			                          outer_border.Max.y, outer_col,
			                          border_size );
		}
		else if( table->Flags & ImGuiTableFlags_BordersOuterH )
		{
			inner_drawlist->AddLineH( outer_border.Min.x, outer_border.Max.x,
			                          outer_border.Min.y, outer_col,
			                          border_size );
			inner_drawlist->AddLineH( outer_border.Min.x, outer_border.Max.x,
			                          outer_border.Max.y, outer_col,
			                          border_size );
	    ...
	    table->RowPosY2 < table->OuterRect.Max.y )
	{
		...
		if( border_y >= table->BgClipRect.Min.y &&
			...
			inner_drawlist->AddLineH( table->BorderX1, table->BorderX2,
			                          border_y, table->BorderColorLight,
			                          border_size );
	}
...
TableGetColumnAvailSortDirection( ImGuiTableColumn* column, int n )
{
	...
	return (
	    ImGuiSortDirection)( ( column->SortDirectionsAvailList >> ( n << 1 ) ) &
	                         0x03 );
}
                                         ...
                                         bool append_to_sort_specs )
{
	...
	if( append_to_sort_specs )
		for( int other_column_n = 0; other_column_n < table->ColumnsCount;
		     other_column_n++ )
			sort_order_max = ImMax( sort_order_max,
...
void ImGui::TableHeadersRow()
{
	...
	if( table
	        ->HostSkipItems ) // Merely an optimization, you may skip in your own code.
		return;
...
void ImGui::TableHeader( const char* label )
{
	...
	// [RTL] Right-align the header label within the cell.
	if( ( g.IO.ConfigFlags & ImGuiConfigFlags_RightToLeft ) != 0 )
		label_pos.x = ImMax( label_pos.x, ellipsis_max - label_size.x );
		...
		for( int order_n = 0; order_n < data_count; order_n++ )
		{
			...
			if( pass == 0 )
			{
				...
				float line_off_for_align_x =
				    ImFloor( ImMax( ( ( ( column->MaxX - column->MinX ) -
				                        padding.x * 2.0f ) -
				...
				while( label_name < label_name_end )
				{
					...
					float clip_width =
					    max_label_width -
					    padding
					        .y; // Using padding.y*2.0f would be symmetrical but hide more text.
...
static bool MenuItemForColumnReorder( const char* label, bool selected,
                                      bool enabled )
{
	...
	float             min_w       = offsets->DeclColumns(
	    0.0f, label_size.x, 0.0f, checkmark_w ); // Feedback for next frame
	...
	if( g.ActiveId == id )
		selectable_flags |=
		    ImGuiSelectableFlags_Highlight; // Stays highlighted while dragging.
	const bool has_been_moved =
	    ( g.ActiveId == id ) &&
	    g.ActiveIdHasBeenEditedBefore; // But disable toggling once moved.

	BeginDisabled(
	    !enabled ); // Don't use ImGuiSelectableFlags_Disabled so that Check mark is also affected.
	bool ret =
	    Selectable( label, false, selectable_flags,
    ...
    ImGuiTable* table, ImGuiTableFlags flags_for_section_to_display )
{
	...
	if( flags_for_section_to_display &
		...
		if( BeginMenu( LocalizeGetMsg( ImGuiLocKey_TableReset ) ) )
		{
			...
			if( flags_for_section_to_display & ImGuiTableFlags_Reorderable )
				if( MenuItem(
				        LocalizeGetMsg( ImGuiLocKey_TableResetOrder ), NULL,
				        false,
				        !table
				             ->IsDefaultDisplayOrder ) ) // PS: cannot be hidden because it would mess with drag reordering.
					table->IsResetDisplayOrderRequest = true;
			if( flags_for_section_to_display & ImGuiTableFlags_Hideable )
				if( MenuItem(
				        LocalizeGetMsg( ImGuiLocKey_TableResetVisibility ),
				        NULL, false, !table->IsDefaultVisibility ) )
	...
	if( flags_for_section_to_display & ImGuiTableFlags_Hideable )
	{
		...
		const int reorder_max_order =
		    is_reordering
		        ? TableGetMaxDisplayOrderAllowed( table, reorder_src_order,
		...
		for( int order_n = 0; order_n < table->ColumnsCount; order_n++ )
		{
			...
			if( is_reordering && ( column->DisplayOrder < reorder_min_order ||
				...
				menu_item_enabled = false;
			if( MenuItemForColumnReorder( name, column->IsUserEnabled,
			                              menu_item_enabled ) )
			    ...
			    ( table->Flags & ImGuiTableFlags_Reorderable ) )
			{
				g.ActiveIdHasBeenEditedBefore =
				    true; // Disable toggle in MenuItemForColumnReorder() + start dimming to display allowed reorder targets.
				table->ReorderColumn = (ImGuiTableColumnIdx)column_n;
				if( !IsItemHovered() )
				{
					int reorder_dir =
...
void ImGui::TableLoadSettings( ImGuiTable* table )
{
	...
	if( table->SettingsOffset == -1 )
	{
		...
		if( settings->ColumnsCount !=
		    table
		        ->ColumnsCount ) // Allow settings if columns count changed. We could otherwise decide to return...
			table->IsSettingsDirty = true;
		table->SettingsOffset = g.SettingsTables.offset_from_ptr( settings );
	}
	else
...
void ImGui::TableLoadSettingsForColumns( ImGuiTable* table )
{
	...
	for( int n = 0; n < table->ColumnsCount; n++ )
	{
		if( n >= settings->ColumnsCount ||
			...
			break;
		TableLoadSettingsForColumn( &table->Columns[n], &column_settings[n],
		                            settings->SaveFlags );
		matches++;
	}
	...
	for( ImGuiTableColumn& column : table->Columns )
		if( column.ID != 0 && !column.IsLoadedSettings )
			for( int n = settings_start_n; n < settings->ColumnsCount; n++ )
				...
				{
					TableLoadSettingsForColumn( &column, &column_settings[n],
					                            settings->SaveFlags );
					matches++;
					break;
				}
...
void ImGui::TableLoadSettingsForColumn(
    ImGuiTableColumn* column, ImGuiTableColumnSettings* column_settings,
    ImGuiTableFlags load_flags )
{
	...
	if( load_flags & ImGuiTableFlags_Reorderable )
		...
		column->DisplayOrder =
		    column_settings
		        ->Index; // Because default depends on previous Index, we need to set that up and cannot rely on TableInitColumnDefaults()
...
void ImGui::TableFixDisplayOrder( ImGuiTable* table )
{
	...
	g.TempBuffer.reserve( (
	    int)( sizeof( ImGuiTableFixDisplayOrderColumnData ) *
	          table
	              ->ColumnsCount ) ); // FIXME: Maybe wrap those two lines as a helper.
                                          ...
                                          ImGuiSettingsCleanupArgs* args )
{
	     ...
	     settings != NULL; settings = g.SettingsTables.next_chunk( settings ) )
	{
		...
		if( args->SetCurrentSessionDateToAll ||
		    ( args->SetCurrentSessionDateWhenMissingDate &&
...
void ImGui::DebugNodeTable( ImGuiTable* table )
{
	...
	BulletText( "ResizedColumn: %d, HeldHeaderColumn: %d, ReorderColumn: %d",
	            table->LastResizedColumn, table->LastHeldHeaderColumn,
	            table->ReorderColumn );
                                    ...
                                    ImGuiTable*         table )
{
	if( settings->ID == 0 )
		PushID( settings );
	const bool open =
	    TreeNode( (void*)(intptr_t)settings->ID, "Settings 0x%08X (%d columns)",
...
void ImGui::EndColumns()
{
	...
	if( !( flags & ImGuiOldColumnFlags_GrowParentContentsSize ) )
		window->DC.CursorMaxPos.x =
		    columns
		        ->HostCursorMaxPosX; // Restore cursor max pos, as columns don't grow parent
